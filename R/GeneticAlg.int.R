function (genomeLen, codonMin, codonMax, genomeMin = rep.int(codonMin, genomeLen), genomeMax = rep.int(codonMax, genomeLen), suggestions = NULL, popSize = 50, iterations = 100, terminationCost = NA, mutationChance = 1/(genomeLen + 1), elitism = floor(popSize/10), geneCrossoverPoints = NULL, monitorFunc = NULL, evalFunc, allowrepeat = TRUE, showSettings = FALSE, verbose = FALSE, plapply = lapply) 
{
    ...
    if (showSettings) {
        ...
        result = list(genomeMin = genomeMin, genomeMax = genomeMax, suggestions = suggestions, popSize = popSize, iterations = iterations, elitism = elitism, mutationChance = mutationChance)
        class(result) = "rbga"
        cat(summary(result))
    }
    else {
    ...
    for (iter in 1:iterations) {
        ...
        collect.results <- function() {
            settings = list(genomeMin = genomeMin, genomeMax = genomeMax, popSize = popSize, elitism = elitism, geneCrossoverPoints = geneCrossoverPoints, iterations = iterations, suggestions = suggestions, mutationChance = mutationChance)
        ...
        for (child in (elitism + 1):popSize) {
            ...
            if (is.null(geneCrossoverPoints)) {
                ...
                if (crossOverPoint == 0) {
                  ...
                  newPopulation[child, ] = c(parents[1, 1:crossOverPoint], parents[2, (crossOverPoint + 1):genomeLen])
                }
            }
            else {

function (inputString, grammar, wrappings = 3) 
{
    ...
    df_currentIteration = data.frame(Step = 0, Codon = NA, Symbol = "", Rule = "starting:", Result = startSymb)
    df = NULL
    step = 1
    for (i in 1:wrappings) {
        ...
        for (j in 1:total.genes) {
            ...
            df_currentIteration = rbind(df_currentIteration, data.frame(Step = step, Codon = codon.string[j], Symbol = paste0("<", non.terminal.symb, ">"), Rule = chosen.rule, Result = res))
            if (res == startSymb) {
                break
            }
            startSymb = res
